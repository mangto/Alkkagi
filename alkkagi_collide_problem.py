import pygame, sys, os, json, win32api
from pygame import gfxdraw

pygame.init()

window = pygame.display.set_mode((960, 960))
pygame.display.set_caption("Alkkagi Alpha Ver.")
...
class al:
    ...
    def draw(self, **settings):
        ...
        if (fps>0 and self.moving):
            self.speed = (self.speed[0]-self.cos*self.friction*100/fps, self.speed[1]+self.sin*self.friction*100/fps)
        ...
        if (pygame.mouse.get_pressed()[0] == 0):
            if (self.activated):
                self.speed = (multiplier*cos*self.SpeedMultiplier, -1*multiplier*sin*self.SpeedMultiplier)
        ...
        for object in system.objects:
            ...
            if (d <= self.size * 2):
                object.moving = True
                object.speed = (self.speed[0]/2, self.speed[1]/2)
                object.cos = self.cos
                object.sin = self.sin
                object.speed = (object.speed[0]-object.cos*object.friction*60/fps, object.speed[1]+object.sin*object.friction*60/fps)
                object.pos = (object.pos[0]+object.speed[0]/fps*object.SpeedMultiplier, object.pos[1]+object.speed[1]/fps*object.SpeedMultiplier)

                self.cos *= -1
                self.sin *= -1
                self.speed = (-1*self.speed[0]/2, -1*self.speed[1]/2)
                self.speed = (self.speed[0]-self.cos*self.friction*60/fps, self.speed[1]+self.sin*self.friction*60/fps)
                self.pos = (self.pos[0]+self.speed[0]/fps*self.SpeedMultiplier, self.pos[1]+self.speed[1]/fps*self.SpeedMultiplier)
        return
